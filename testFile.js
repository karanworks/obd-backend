...
async function testFunction() {
  ...
  console.log("ACTIVE CAMPAIGN IDS ->", activeCampaignsIds);
  ...
  const data = await prisma.campaignDataSetting.findMany({
    where: {
      campaignId: {
  ...
  const connectedNumbers = await prisma.callResponseCDR
    .findMany({
      where: {
        connectedlinenum: {
          in: await prisma.campaignDialingData
            .findMany({
              where: { campaignId: 61 },
              select: { id: true },
            })
            .then((res) => res.map((record) => parseInt(record.id))),
        },
      },
      select: { connectedlinenum: true },
    })
    .then((res) => res.map((record) => record.connectedlinenum));

  const campaignDialingData2 = await prisma.campaignDialingData.findMany({
    where: {
      id: {
        notIn: connectedNumbers,
      },
    },
  });

  console.log("TESTING DATA ->", campaignDialingData2);
